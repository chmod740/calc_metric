function [pt,pitchMask] = postPitchProcess_ke(pitchContourIn,pitchMask0,nChan, sex)

% pRange = [55,100]; %Ke uses this

% if ~exist('thr','var'); thr = -0.1; end;
...
if strcmp(sex,'f')
   pRange = [40,140];  %Guoning uses this
%    pRange = [10,90];  %Guoning uses this
elseif strcmp(sex,'m')
%     pRange = [60 180];
%       pRange = [60 230];
%       pRange = [50 260];
%       pRange = [40 110]; %
      pRange = [40 230]; % use this a lot
%       pRange = [60 270]; % same as the other grping
%     pRange = [80 250];
%     pRange = [40 130];
end
...
%% added for CID
% pitchMask = (pitchmask>thr);
% idxv = pt >0;
% pitchMask = pitchMask';
% pitchMask(:,~idxv) = repmat(zeros(128,1),1,sum(~idxv));
% % pitchMask(:,end) = zeros(128,1);
% pitchMask = pitchMask(:,1:end-1);
%%
fprintf('done.\n');
