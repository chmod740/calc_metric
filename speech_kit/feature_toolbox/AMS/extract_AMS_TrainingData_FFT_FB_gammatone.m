function [ns_ams, true_SNR] = extract_AMS_TrainingData_FFT_FB_gammatone(cl, tn, nChnl, freq)
...
for kk=1:KK
    if kk == 1
        ns_env_frm = ns_env(:,(1:AMS_frame_step)+(AMS_frame_step*(kk-1)));
        ams = abs(fft(ns_env_frm'.*repwin_ams((Srate/len2*s_frame_len/1000/2+1):end,:),nFFT_ams));
        
        tn_env_frm = tn_env(:,(1:AMS_frame_step)+(AMS_frame_step*(kk-1)));
        cl_env_frm = cl_env(:,(1:AMS_frame_step)+(AMS_frame_step*(kk-1)));        
    else
        ns_env_frm = ns_env(:,(1:AMS_frame_len)+(AMS_frame_step*(kk-2)));    
