function [cepstra, spectra, pspectrum, lpcas, F, M] = rastaplp(samples, sr, dorasta, modelorder,wintime)
...
pspectrum = powspec(samples, sr,wintime);
...
%  nl_aspectrum = (aspectrum); disp('rasta in pwspec domain...')
...
%  aspectrum = abs(ras_nl_aspectrum)+1; 

end
  
% do final auditory compressions
postspectrum = postaud(aspectrum, sr);
% postspectrum = postaud(aspectrum, sr/2);
